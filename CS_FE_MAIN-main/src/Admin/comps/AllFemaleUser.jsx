...
const AllFemaleUser = () => {
  ...
  const fetchTotalFemaleUsers = async (page = 1, limit = perPage) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-female-users", {
        params: { page, limit },
      });
      setFemaleUsers(response.data.users);
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalFemaleUsers(pageNumber);
  };
...
const FemaleData = ({ femaleUsers, currentPage, perPage }) => {
  return (
