...
const DeletedUsers = () => {
  ...
  const fetchTotalDeletedUsers = async (page = 1, limit) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-deleted-users", {
        params: { page, limit },
      });
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalDeletedUsers(pageNumber);
  };
  useEffect(() => {
...
const FemaleData = ({ deletedUsers, currentPage, perPage }) => {
  return (
