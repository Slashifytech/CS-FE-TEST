...
const ActiveUser = () => {
  ...
  const fetchTotalActiveUsers = async (page = 1, limit = perPage) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-active-users", {
        params: { page, limit },
      });
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalActiveUsers(pageNumber);
  };
  ...
  return (
    <>
      ...
      <div className="w-[100%] md:w-[100%] sm:w-[100%] overflow-x-scroll scrollbar-hide">
        <div className="flex flex-row justify-between md:mx-36 mx-6 md:ml-[27%] sm:ml-[40%] w-[160%]  md:w-[65%] sm:w-[90%] font-semibold text-center text-[17px] items-start font-DMsans mb-6">
...
const ActiveData = ({ activeUsers, currentPage, perPage }) => {
  //   console.log(femaleUsers, "ddn");
  return (
    <div className="flex flex-col md:mx-40  md:ml-[27%] ml-[8%] sm:ml-[41%] mx-6  md:w-[60%] w-[160%] sm:w-[90%] font-normal text-black text-[15px] items-start  font-DMsans mb-9">
