...
import {   curve, logo, warn } from "../../../assets/index";
...
const requested =  userData?.deletedStatus === "This profile has been deleted. User request for re-approval." ? "yes" : "no"
  const deleteData = async () => {
    if (userId) {
      try {
       
        apiurl.put(`/user-reapproval-request`, {
          userId,
...
<div className="absolute pt-9 md:block sm:hidden hidden ">
      ...
      <div className="bg-[#FCFCFC] md:absolute z-30   md:mx-96 mx-6 mt-9 md:mt-0 my-2  pb-6 rounded-xl shadow  flex flex-col justify-center items-center font-DMsans">
        <img src={curve} alt="img" className="sm:w-full xl:w-full" />
        <img src={warn} alt="img" className="w-[14%] xl:w-28 sm:w-[14%] md:w-[14%] mt-12" />
        {
           userData?.deletedStatus === "This profile has been deleted. User request for re-approval." && userData?.registrationPhase === "notapproved" ? 
             <p className="text-center md:px-12 mx-3 pt-5 sm:px-20">
          You have already requested for re-approval of your profile, please wait for admin approval.
        </p>
