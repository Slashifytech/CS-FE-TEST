...
const CategoryAUser = () => {
  ...
  const [categoryAUsers, setCategoryAUsers] = useState([]);
  ...
  // Parse page number from URL parameters
  const getPageFromURL = () => {
    const searchParams = new URLSearchParams(location.search);
    return parseInt(searchParams.get("page")) || 1;
  };

  const fetchTotalCategoryAUsers = async (page = 1, limit = perPage) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-users-category-a", {
        params: { page, limit },
      });
      setCategoryAUsers(response.data.users);
      ...
      console.error("Error fetching total category A users:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalCategoryAUsers(pageNumber);
  };

  useEffect(() => {
    const page = getPageFromURL();
    fetchTotalCategoryAUsers(page);
  }, [location.search]);
...
const CategoryAData = ({ categoryAUsers, currentPage, perPage }) => {
  return (
