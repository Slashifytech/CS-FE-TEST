...
const AllMaleUsers = () => {
  ...
  const fetchTotalMaleUsers = async (page = 1, limit) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-male-users", {
        params: { page, limit },
      });
      setMaleUsers(response.data.users);
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalMaleUsers(pageNumber);
  };
  useEffect(() => {
    fetchTotalMaleUsers();
  }, []);
    ...
    <>
      ...
      <div className="w-[100%] md:w-[100%] sm:w-[100%] overflow-x-scroll scrollbar-hide">
        <div className="flex flex-row justify-between md:mx-36 mx-6 md:ml-[27%] sm:ml-[40%] w-[220%]  md:w-[65%] sm:w-[90%] font-semibold text-center text-[17px] items-start font-DMsans mb-6">
          <p className="md:w-5 w-9">S.No</p>
          <p className="text-start  md:w-16 w-20">User Id</p>
...
const FemaleData = ({ maleUsers, currentPage, perPage }) => {
  return (
    <div className="flex flex-col md:mx-40  md:ml-[27%] sm:ml-[41%] ml-[8%] mx-6  md:w-[60%] w-[220%] sm:w-[90%] font-normal text-black text-[15px] items-start  font-DMsans mb-9">
