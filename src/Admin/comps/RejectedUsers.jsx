...
const RejectedUsers = () => {
  const [rejectedUsers, serRejectedUsers] = useState([]);
  ...
  const fetchTotalrejectedUsers = async (page = 1, limit) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-rejected-users", {
        params: { page, limit },
      });
      serRejectedUsers(response.data.users);
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalrejectedUsers(pageNumber);
  };
  useEffect(() => {
    fetchTotalrejectedUsers();
  }, []);
  return (
    <>
      <div className="fixed">
...
const RejectedData = ({ rejectedUsers, currentPage, perPage }) => {
  console.log(rejectedUsers, "lml");
  return (
