...
const CategoryUnUser = () => {
  const [categoryUnUsers, setCategoryUnUsers] = useState([]);
  ...
  const fetchTotalCategoryUnUsers = async (page = 1, limit = perPage) => {
    try {
      setLoading(true);
      const response = await apiurl.get("/total-users-uncategorised", {
        params: { page, limit },
      });
      setCategoryUnUsers(response.data.users);
  ...
  const handlePageChange = (pageNumber) => {
    fetchTotalCategoryUnUsers(pageNumber);
  };

  useEffect(() => {
    fetchTotalCategoryUnUsers();
  }, []);
    ...
    <>
      ...
      <div className="w-[100%] md:w-[100%] sm:w-[100%] overflow-x-scroll scrollbar-hide">
        ...
        {loading ? (
          ...
          <CategoryUnData
            categoryUnUsers={categoryUnUsers}
...
const CategoryUnData = ({ categoryUnUsers, currentPage, perPage }) => {
  return (
    <div className="flex flex-col md:mx-40  md:ml-[27%] ml-[8%] sm:ml-[41%] mx-6  md:w-[60%] w-[220%] sm:w-[90%] font-normal text-black text-[15px] items-start  font-DMsans mb-9">
      {categoryUnUsers?.map((item, index) => (
        <div
          key={item.userId}
